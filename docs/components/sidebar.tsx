    ...
    return (
        <aside className="w-1/5 absolute left-0 top-16">
            <Accordion type="multiple">
                {pkg.packages && pkg.packages.length > 0 && <AccordionItem value="Packages" className="w-full px-4">
                    <AccordionTrigger>Packages</AccordionTrigger>
                    <AccordionContent>
                        {pkg.packages.map((p) => (
                            <div key={p.name} className="p-1">
                                <Link key={p.name} className="pl-4 capitalize" href={findPackagePath(p.name)}>{p.name}</Link>
                            </div>
                        ))}
                    </AccordionContent>
                </AccordionItem>}
                {pkg.modules && pkg.modules.length > 0 && <AccordionItem value="Modules" className="w-full px-4">
