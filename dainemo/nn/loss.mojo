...
struct CrossEntropyLoss:
    ...
    fn forward(inout self, inout outputs: Node[dtype], targets: Tensor[dtype]) -> Node[dtype]:
        ...
        alias epsilon = 1/max_finite[dtype]()
...
struct BCELoss:
    ...
    fn forward(inout self, inout outputs: Node[dtype], targets: Tensor[dtype]) -> Node[dtype]:
        ...
        let logout = LOG.forward(outputs)
        let targets_logout = MUL.forward(Node[dtype](targets), logout)
        let logout_1min = LOG.forward(SUB.forward(Node[dtype](1), outputs))
        let targets_1min = SUB.forward(Node[dtype](1), Node[dtype](targets))
